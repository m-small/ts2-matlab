function [z_pred,yp,ep,y_data,B,Bsc,Bpr,C,Csc,Cpr]=cva_analysis2(data,td,ps,emb,bnp,cnp)
...
%build the B matrix
disp(']] [B,Bsc,Bpr,yp,yt,ep,pl]=cva_sensitive(z_pred,y_data,bnp,ps);');
[B,Bsc,Bpr,yp,y_data,ep,z_pred]=cva_sensitive(z_pred,y_data,bnp,ps);                

%build the C matrix
e_data=abs(ep);
disp(']] [C,Csc,Cpr,ep,e_data,eep,pl]=cva_sensitive(z_pred,e_data,cnp,ps);');
[C,Csc,Cpr,ep,e_data,eep,z_pred]=cva_sensitive(z_pred,e_data,cnp,ps);
clear e_data eep
...
if trace,
  ...
  disp(']] cva_movie(yp,ep,y_data,z_pred,length(y_data));');
  cva_movie(yp,ep,y_data,z_pred,length(y_data));
  close;
end;
