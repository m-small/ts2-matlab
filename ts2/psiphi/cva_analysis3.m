...
%embed the fit data
disp(']] [z_pred,y_data,x_data]=cva_makeit(fit,ps,emb);');
[z_pred,y_data,x_data]=cva_makeit(fit,ps,emb);
clear fit

%build the B matrix
disp(']] [B,Bsc,yp,yt,ep,pl]=cva2(z_pred,y_data,bnp,ps,x_data);');
[B,Bsc,yp,y_data,ep,z_pred]=cva2(z_pred,y_data,bnp,ps,x_data);                

%build the C matrix
e_data=abs(ep);
disp(']] [C,Csc,ep,e_data,eep,pl]=cva2(z_pred,e_data,cnp,ps,x_data);');
[C,Csc,ep,e_data,eep,z_pred]=cva2(z_pred,e_data,cnp,ps,x_data);
clear e_data eep
...
%and embed it
disp(']] [z_pred,y_data,x_data]=cva_makeit(test,ps,emb);');
[z_pred,y_data,x_data]=cva_makeit(test,ps,emb);
clear test ps emb

%and make the predictions
disp(']] y_pred=cva_pred2(z_pred,B,Bsc,bnp,x_data);');
yp=cva_pred2(z_pred,B,Bsc,bnp,x_data);
disp(']] e_pred=cva_pred2(z_pred,C,Csc,cnp,x_data);');
ep=cva_pred2(z_pred,C,Csc,cnp,x_data);
clear bnp cnp
