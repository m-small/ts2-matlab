...
C-------------------------------------------------------------------------------
C
C	This is a mex file for MATLAB. It calculates the derivative of the 
C	cardinal B-wavelet  function for a given (vector) set of parameters 
C	(for a vector of input values), used to specify the dilation, translation 
C	and order parameter of the function. The function is normalised to have 
...
C
C	MATLAB call: y = dbsplinewave(x,pms);
C
C	 INPUT:
...
C	Note: ORD_MAX id defined in function bspline too
C-------------------------------------------------------------------------------

	real*8  y(NMAX), x(NMAX), pms(NO_PARAMS,NMAX)
	real*8  qn(3*ORD_MAX-1)
	integer ord, tot
	
	real*8  xt1, xt2, temp
	integer i,l,so_pr,qn_pr
...
C-------------------------------------------------------------------------------
	         xt1 = (2*x(i)-(2*pms(2,i)+(l-1)*pms(1,i)))/pms(1,i)
	         xt2 = (2*x(i)-(2*pms(2,i)+l*pms(1,i)))/pms(1,i)
	      elseif ((size_pms.NE.1) .AND. (n.EQ.1)) then
C-------------------------------------------------------------------------------
C	      Centre Functions
	         if (l.EQ.1) x(i) = x(i) + (pms(1,1)*(2*ord-1))/2.0
C-------------------------------------------------------------------------------
	         xt1 = (2*x(i)-(2*pms(2,1)+(l-1)*pms(1,1)))/pms(1,1)
	         xt2 = (2*x(i)-(2*pms(2,1)+l*pms(1,1)))/pms(1,1)
	      else
C-------------------------------------------------------------------------------
C	      Centre Functions
...
C-------------------------------------------------------------------------------
	      ...
	      y(i) = y(i) + 2*qn(l)*(bspline(xt1,ord-1) - bspline(xt2,ord-1))
	      
20	   continue

	   if ((size_pms.NE.1) .AND. (n.NE.1)) then
	      y(i) = (1/(pms(1,i)*sqrt(pms(1,i))))*y(i)
	   elseif ((size_pms.NE.1) .AND. (n.EQ.1)) then
	      y(i) = (1/(pms(1,1)*sqrt(pms(1,1))))*y(i)
	   endif
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	real*8 FUNCTION bspline(x,ord)
	
	integer ord, k, p, q, ORD_MAX
	PARAMETER (ORD_MAX = 99)
	real*8  x1, x, y, c(ORD_MAX-1)
...
10	   continue
	   ...
	   do 20 p = 1,ord-2
	      do 30 q = 1,ord-1-p
	         c(q) = ((x-q+1) * c(q)+(p+q+1-x)*c(q+1)) / (p+1)
30	      continue
20	   continue
