function bifurcation2(y_fit,mu_fit,y_test,mu_test,v,func,penalty,niterat,nbuilds,nbasis,nexpand,wobble,meth);
...
%
% modified to accept predifined value of the bifurcation parameter
% mu (instead of assuming it is linear in time).
%
% M. Small 
% Created: 6/6/99
% Updated: 4/7/00
...
if isempty(X),
  ...
  [dmu,nmu]=size(mu_fit);
  if dmu~=1,
    disp('mu_ft must be a row vector');
  end;
  if nmu~=nx,
    disp('length of mu_fit and size of X don''t agree');
  end;
  clear dx nx dmu nmu
  X=[X;mu_fit];
end;
...
if isempty(y_test),
   ...
   [yt,yp,ep]=bfpredict(y_fit,mu_fit);
   subplot(211);
   plotcols(ep,'b:',yt,'k-',yp,'r--');
   zoom on;
   title(['dishonest predictions RMS = ',num2str(rms(ep))]);
   %honest predictions
   mutest=mu_test;%(mu(end)+diff(mu(1:2))):diff(mu(1:2)):(mu(end)+diff(mu(1:2))*length(y_test));
   ...
   %no-noise
   mutest=[mu_test(1) mu_test(2)-mu_test(1)];
